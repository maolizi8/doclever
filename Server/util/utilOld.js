...
var runTestCode=async function (code,test,global,opt,root,argv,mode,__id,level,testid) {
    if(!testModel)
    {
    ...
    // logger.debug('server>util.js>runTestCode-1698>text')
    // logger.debug(text)
...
let runPoll=async function (arr) {
    ...
    for(let obj of arr)
    {
        ...
        if(arr.length>0)
        {
            let subject="[DOClever]"+moment().format("YYYY-MM-DD HH:mm:ss")+" 项目"+obj.project.name+"定时任务结果";
        ...
        if(obj.phoneInfo && obj.phoneInfo.baseUrl && obj.phoneInfo.contentParam && obj.phoneInfo.sign)
        {
            ...
            param[obj.phoneInfo.contentParam]=encodeURI(`测试共(${root.count}), 成功(${root.success}), 失败(${root.fail}), 未判定(${root.unknown}), ${obj.phoneInfo.sign}`)
...
var runInterface2=async function (obj,global,test,root,opt,level,pollTest,testInterfaces) {
    ...
    logger.debug('<runInterface2>-开始运行接口：'+obj.name)
	
	
	testInterfaces[resultTestId].push({
		interId:obj._id,
		interName:obj.name,
		interBaseUrl:obj.baseurl?obj.baseurl:"",
		interPath:obj.url
    })
    var testIdLength=testInterfaces[resultTestId].length;
    ...
    if(!baseUrl)
    {
        ...
        //     interBaseUrl:obj.baseurl?obj.baseurl:"",
        //     interPath:obj.url,
        //     errMessage:"未设置baseUrl"
        // })
        testInterfaces[resultTestId][testIdLength-1].errMessage="未设置baseUrl"
    ...
    logger.debug('<runInterface2>-objReq');
    logger.debug(objReq);
    testInterfaces[resultTestId][testIdLength-1].request=interRequest;
                                                        // {
                                                        //     url:objReq.url,
                                                        //     method:objReq.method,
                                                        //     headers:JSON.stringify(objReq.headers),
    ...
    return func.then(function (result) {
        ...
        testInterfaces[resultTestId][testIdLength-1].result={
                                                            headers:JSON.stringify(res.header),
                                                            status:res.status,
                                                            bodytype:res.type,
                                                            data:res.data
                                                        };
        testInterfaces[resultTestId][testIdLength-1].runTime=res.second;
        // testInterfaces[resultTestId].push({
        //     interId:obj._id,
        //     interName:obj.name,
        //     interBaseUrl:obj.baseurl?obj.baseurl:"",
        ...
        // logger.debug('<runInterface2>-cookies');
        // logger.debug(cookies);
        // logger.debug('<runInterface2>-res.cookies');
        ...
        logger.debug('<runInterface2>-err');
        logger.error(err);

        root.output+=err.message+"<br>";
        let interRunTime=(((new Date())-startDate)/1000).toFixed(3);
        
        testInterfaces[resultTestId][testIdLength-1].errMessage=err.message;
        testInterfaces[resultTestId][testIdLength-1].runTime=interRunTime;
        // testInterfaces[resultTestId].push({
        //     interId:obj._id,
        //     interName:obj.name,
        //     interBaseUrl:obj.baseurl?obj.baseurl:"",
        //     interPath:obj.url,
        //     runTime:interRunTime,
...
var runTestCode2=async function (code,test,global,opt,root,argv,mode,__id,level,pollTest,testInterfaces,testTests) {
    logger.debug('runTestCode2>>>>');
	logger.debug('runTestCode2>>>>__id');
    logger.debug(__id);
	logger.info('runTestCode2>>>>level');
    logger.info(level);
    //logger.debug('runTestCode2>>>>root');
    //logger.debug(root);
    logger.info('runTestCode2>>>>test');
    logger.info(test);
   ...
   // var Base64=BASE64.encoder,MD5=CryptoJS.MD5,SHA1=CryptoJS.SHA1,SHA256=CryptoJS.SHA256,SHA512=CryptoJS.SHA512,SHA3=CryptoJS.SHA3,RIPEMD160=CryptoJS.RIPEMD160,AES=CryptoJS.AES.encrypt,TripleDES=CryptoJS.TripleDES.encrypt,DES=CryptoJS.DES.encrypt,Rabbit=CryptoJS.Rabbit.encrypt,RC4=CryptoJS.RC4.encrypt,RC4Drop=CryptoJS.RC4Drop.encrypt;
    ...
    logger.debug('runTestCode2>arr')
    logger.debug(arr)

    for(var i=0;i<arr.length;i++)
    {
        logger.debug('runTestCode2>for-----------------'+i)
        ...
        logger.debug('<runTestCode2>type>')
        ...
        {	
            ...
            let objInfo={};
            let o=JSON.parse(obj.replace(/\r|\n/g,""));
            logger.debug('runTestCode2>type=1>o(interobj before update)')
            logger.debug(o);
            // logger.debug('runTestCode2>type=1>testObj')
            ...
            // logger.debug('runTestCode2>type=1>o(interobj after update)')
            // logger.debug(o);
            ...
            if(!bFind)
            {
                ...
                logger.debug('runTestCode2>type=1>objInfo')
            ...
            if(o.example)
            {
                try
                {
                    ...
                    //,"param"
                    // logger.debug('runTestCode2>type=1>objExample.param')
                ...
                {
                    logger.error('runTestCode2>type=1>example')
                    logger.error(err)
                }
            }
            ...
            logger.debug('runTestCode2>type=1>opt')
            logger.debug(opt)
            // logger.debug('runTestCode2>type=1>obj')
            // logger.debug(obj)

            var formatObj=obj.replace(/\r|\n/g,"")
			
			/* logger.debug('runTestCode2>type=1>before runInterface2>test.module')
            logger.debug(test.module)
			 */
			logger.debug('runTestCode2>type=1>before runInterface2>formatObj')
            logger.debug(formatObj)
        ...
        else if(type=="2")
        {
			...
			logger.debug('runTestCode2>type=2>testObj')
            logger.debug(testObj);
            ...
            logger.debug('runTestCode2>type=2>testObj')
            logger.debug(testObj);
            //logger.debug('runTestCode2>type=2>testMode')
            //logger.debug(testMode);
			logger.debug('runTestCode2>type=2>before next runTestCode2>test.module')
            logger.debug(test.module);
			logger.debug('runTestCode2>type=2>before next runTestCode2>testObj.name')
            logger.debug(testObj.name);
			
			
			logger.debug('runTestCode2>type=2>before next runTestCode2>testTests')
            ...
            logger.debug('runTestCode2>type=2>code')
            //logger.debug(code);
            text="(function () {var argv=Array.prototype.slice.call(arguments);return runTestCode2('"+code+"',"+JSON.stringify(testObj)+",global,opt,root,argv,'"+testMode+"',"+(level==0?objId:undefined)+","+(level+1)+",pollTest,testInterfaces,testTests)})"
        ...
        // logger.debug('runTestCode2>for>text')
        // logger.debug(text)

        var node=document.createTextNode(text);

        logger.debug('runTestCode2>createTextNode>node')
        logger.debug(node);
    ...
    logger.debug('runTestCode2>arrNode')
    ...
    {   
		...
		if(level==1){
			resultTestId=test._id
			testTests[resultTestId]=[]
			testInterfaces[resultTestId]=[]
		}else if(level>1){
			testTests[resultTestId].push({
        ...
        logger.debug('runTestCode2>开始执行用例'+test.module.name+"/"+test.group.name+"/"+test.name+"("+(mode=="code"?"代码模式":"UI模式")+")")
        
		root.output+="<br><div style='background-color: #ececec'>["+moment().format("YYYY-MM-DD HH:mm:ss")+"]开始执行用例："+test.module.name+"/"+test.group.name+"/"+test.name+"("+(mode=="code"?"代码模式":"UI模式")+")<br>";
    }
    else
    {
        logger.debug('runTestCode2>开始执行定时任务')
        startDate=new Date();
    ...
    logger.debug('server>util.js>runTestCode2>text')
    logger.debug(text)
    ...
    var ret=eval("(async function () {"+text+"})()").then(function (ret) {
        ...
        logger.debug('runTestCode2>eval：typeof(ret)')
        logger.debug(ret)
        ...
        if(temp===undefined)
        {
            ...
            if(test.module)
            {
				logger.debug('runTestCode2>for>testend>(if)testInterfaces')
				logger.debug(testInterfaces)
				logger.debug('runTestCode2>for>testend>(if)testTests')
				logger.debug(testTests)
                logger.debug('runTestCode2>for>testend>(if)level')
				logger.debug(level)
				...
				if (level==1) {
                    pollTest.push({
                        ...
                        status:test.status,
                        mode:mode,
                        interfaces:testInterfaces[resultTestId]
                    })
                }
				
				logger.debug('runTestCode2>for>testend>(if)pollTest')
				logger.debug(pollTest) 
				
                logger.debug('runTestCode2>用例执行结束：未判定')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]用例执行结束："+test.module.name+"/"+test.group.name+"/"+test.name+"(未判定)";
            }
            else
            {   
                ...
                logger.debug('runTestCode2>定时任务执行结束 if')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]定时任务执行结束";
            }
        }
        else if(Boolean(temp)==true)
        {
            ...
            if(test.module)
            {
				logger.debug('runTestCode2>for>testend>(else if)testInterfaces')
				logger.debug(testInterfaces)
				logger.debug('runTestCode2>for>testend>(else if)testTests')
                logger.debug(testTests)
                logger.debug('runTestCode2>for>testend>(else if)level')
				logger.debug(level)
				...
				logger.debug('runTestCode2>for>testend>(else if)pollTest')
				logger.debug(pollTest) 
				
                logger.debug('runTestCode2>用例执行结束：已通过')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]用例执行结束："+test.module.name+"/"+test.group.name+"/"+test.name+"(<span style='color:green'>已通过</span>)";
            }
            else
            {
                root.pollRunTime=(((new Date())-startDate)/1000).toFixed(3);
                logger.debug('runTestCode2定时任务执行结束 elif')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]定时任务执行结束";
            }
        }
        else
        {
            ...
            if(test.module)
            {
				logger.debug('runTestCode2>for>testend>(else)testInterfaces')
				logger.debug(testInterfaces)
				logger.debug('runTestCode2>for>testend>(else)testTests')
                logger.debug(testTests)
                logger.debug('runTestCode2>for>testend>(else)level')
				logger.debug(level)
				...
				logger.debug('runTestCode2>for>testend>(else)pollTest')
				logger.debug(pollTest) 
				
				
                logger.debug('runTestCode2>用例执行结束：未通过')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]用例执行结束："+test.module.name+"/"+test.group.name+"/"+test.name+"(<span style='color:red'>未通过</span>)";
            }
            else
            {
                root.pollRunTime=(((new Date())-startDate)/1000).toFixed(3);
                logger.debug('runTestCode2>定时任务执行结束 else')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]定时任务执行结束";
            }
        }
		...
		root.output+="</div><br>"
        logger.debug('server>util.js>runTestCode2>obj')
        logger.debug(obj)
        logger.debug('server>util.js>runTestCode2>test')
        //logger.debug(test)
        logger.debug('server>util.js>runTestCode2>root')
        //logger.debug(root)
		logger.debug('server>util.js>runTestCode2>test.module')
        logger.debug(test.module)
		logger.debug('server>util.js>runTestCode2>testInterfaces')
        logger.debug(testInterfaces)
...
var runTestCode2_1=async function (code,test,global,opt,root,argv,mode,__id,level,pollTest,testInterfaces,testTests) {
    logger.debug('runTestCode2>>>>');
	logger.debug('runTestCode2>>>>__id');
    logger.debug(__id);
	logger.info('runTestCode2>>>>level');
...
var runTestCode3=async function (code,test,global,opt,root,argv,mode,__id,level,pollTest,testInterfaces,testTests) {
	...
	logger.debug('runTestCode3>>>>__id');
    logger.debug(__id);
	logger.info('runTestCode3>>>>level');
    logger.info(level);
    //logger.debug('runTestCode3>>>>root');
    //logger.debug(root);
    logger.info('runTestCode3>>>>test');
    logger.info(test);
   ...
   // var Base64=BASE64.encoder,MD5=CryptoJS.MD5,SHA1=CryptoJS.SHA1,SHA256=CryptoJS.SHA256,SHA512=CryptoJS.SHA512,SHA3=CryptoJS.SHA3,RIPEMD160=CryptoJS.RIPEMD160,AES=CryptoJS.AES.encrypt,TripleDES=CryptoJS.TripleDES.encrypt,DES=CryptoJS.DES.encrypt,Rabbit=CryptoJS.Rabbit.encrypt,RC4=CryptoJS.RC4.encrypt,RC4Drop=CryptoJS.RC4Drop.encrypt;
    ...
    for(var i=0;i<arr.length;i++)
    {
        ...
        {	
            ...
            o.project=interinfo.project;
            o.group=interinfo.group;
            // logger.debug('runTestCode3>type=1>o(interobj after update)')
            // logger.debug(o);
            ...
            logger.debug(test.module)
			 */
			logger.debug('runTestCode3>type=1>before runInterface2>formatObj')
            logger.debug(formatObj)
    ...
    {   
        ...
        logger.debug('runTestCode3>开始执行用例'+test.module.name+"/"+test.group.name+"/"+test.name+"("+(mode=="code"?"代码模式":"UI模式")+")")
    ...
    // logger.debug('server>util.js>runTestCode3>before eval>text')
    // logger.debug(text)
    logger.info('server>util.js>runTestCode3>before eval>level')
    logger.info(level)
    logger.debug('server>util.js>runTestCode3>before eval>test')
    logger.debug(test)
    logger.info('server>util.js>runTestCode3>before eval>test.module')
    logger.info(test.module)
    logger.debug('server>util.js>runTestCode3>before eval>testInterfaces[resultTestId]')
    logger.debug(testInterfaces[resultTestId])
    ...
    var ret=eval("(async function () {"+text+"})()").then(function (ret) {
        ...
        logger.debug('runTestCode3>eval：typeof(ret)')
        logger.debug(ret)
        logger.debug(typeof(ret))
        ...
        if(temp===undefined)
        {
            ...
            if(test.module)
            {
				// logger.debug('runTestCode3>for>testend>(if)testInterfaces')
				// logger.debug(testInterfaces)
				// logger.debug('runTestCode3>for>testend>(if)testTests')
				// logger.debug(testTests)
                // logger.debug('runTestCode3>for>testend>(if)level')
				// logger.debug(level)
				...
				// logger.debug('runTestCode3>for>testend>(if)pollTest')
				// logger.debug(pollTest) 
				
                logger.debug('runTestCode3>用例执行结束：未判定')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]用例执行结束："+test.module.name+"/"+test.group.name+"/"+test.name+"(未判定)";
            }
            else
            {   
                ...
                logger.debug('runTestCode3>定时任务执行结束 if')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]定时任务执行结束";
            }
        }
        else if(Boolean(temp)==true)
        {
            ...
            if(test.module)
            {
				// logger.debug('runTestCode3>for>testend>(else if)testInterfaces')
				// logger.debug(testInterfaces)
				// logger.debug('runTestCode3>for>testend>(else if)testTests')
                // logger.debug(testTests)
                // logger.debug('runTestCode3>for>testend>(else if)level')
				// logger.debug(level)
				...
				// logger.debug('runTestCode3>for>testend>(else if)pollTest')
				// logger.debug(pollTest) 
				
                logger.debug('runTestCode3>用例执行结束：已通过')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]用例执行结束："+test.module.name+"/"+test.group.name+"/"+test.name+"(<span style='color:green'>已通过</span>)";
            }
            else
            {
        ...
        else
        {
            ...
            if(test.module)
            {
				// logger.debug('runTestCode3>for>testend>(else)testInterfaces')
				// logger.debug(testInterfaces)
				// logger.debug('runTestCode3>for>testend>(else)testTests')
                // logger.debug(testTests)
                // logger.debug('runTestCode3>for>testend>(else)level')
				// logger.debug(level)
				...
				// logger.debug('runTestCode3>for>testend>(else)pollTest')
				// logger.debug(pollTest) 
            ...
            {
                root.pollRunTime=(((new Date())-startDate)/1000).toFixed(3);
                logger.debug('runTestCode3>定时任务执行结束 else')
                root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]定时任务执行结束";
            }
        }
        ...
        if(test.module)
        {
            if (level==1) {
                ...
                pollTest[pollTestIndex].interfaces=testInterfaces[resultTestId]
            }
        }
        ...
        logger.debug(testInterfaces)
        logger.debug('server>util.js>runTestCode3>in eval>testInterfaces[resultTestId]')
        logger.debug(testInterfaces[resultTestId])
...
let runPollBackend=async function (pollArr) {
    ...
    {   
        logger.debug('runPollBackend>>for>----------------'+pollObj.name)
        ...
        logger.debug('runPollBackend>for>>>arrTest')
        logger.debug(arrTest)

        let str=convertToCode(arrTest);

        logger.debug('runPollBackend>for>>>str')
        logger.debug(str)
         try
        {	
            var runResult=await (exports.runTestCode2(str,{
                name:"",
                status:0
            },{},{
            ...
            logger.debug('runPollBackend>for>>>runTestCode2>>runResult')
            logger.debug(runResult)

        }
        catch(e)
        {
            logger.debug('runPollBackend>for>>>runTestCode2>>e')
            logger.debug(e)
            root.fail++;
            root.output+=e+"<br>"
        } 
        */
        logger.debug('runPollBackend>objCollection.tests')
        logger.debug(objCollection.tests)
        for (let testCase of objCollection.tests) {
            ...
            {
                logger.debug('runPollBackend>for arrTest>>>runTestCode3>>e')
                logger.debug(e)

                var pollTestIndex=pollTest.length-1;
                pollTest[pollTestIndex].message=e.toString();
                pollTest[pollTestIndex].status=2;
                pollTest[pollTestIndex].interfaces=testInterfaces[resultTestId]?testInterfaces[resultTestId]:[];
                root.fail++;
                root.output+=e+"<br>"
            }
        }
        ...
        logger.debug('runPollBackend>for>>>root')
        //logger.debug(root)
        ...
        // let arrProjectUser=pollObj.testProject.users.map(function (obj) {
        //     return obj.toString();
        // })
        // arrProjectUser.unshift(pollObj.testProject.owner.toString());
        // let arr=[],arrUser=[];
        // for(let u of arrPollUser)
        // {
        //     if(arrProjectUser.indexOf(u)>-1)
        ...
        let query={
            ...
            status:root.fail?3:(root.unknown?0:2),   //0-unkown,2-success, 4-fail
        ...
        try
        {
            await (pollRun.createAsync(query));
        }
        catch(e)
        {
        ...
        {
            let subject="[API自动化]-"+pollObj.name+"-测试报告 "+moment().format("YYYY-MM-DD HH:mm:ss");



            //let content=`<h3>测试共(${root.count}), &nbsp;&nbsp;成功(${root.success}), &nbsp;&nbsp;失败(${root.fail}),&nbsp;&nbsp;未判定(${root.unknown})</h3>`+root.output;
            let content=mailContent(query)
...
var mailContent=function(pollInfo){
    ...
    <body>
        ...
        <h2><span style="font-size:80%;font-weight:normal;">测试集合：</span>${pollInfo.projectName}/${pollInfo.collectionName}</h2>
            <table class="table" style="width:300px;">
                ...
                <tr>
                    <td style="color:red;">${pollInfo.testFail||"0"}</td>
                    <td>${pollInfo.testTotal||"0"}</td>
                    <td>${pollInfo.testSuccess||"0"}</td>
                    <td>${pollInfo.testUnkown||"0"}</td>
                </tr>
            </table>
            <div class="clear"></div>
        ...
        for (let j = 0; j < interfaces.length; j++) {
            ...
            if (interfaces[j].result) {
                if (interfaces[j].result.status=="200") {
                    html+=`（状态：<span style="dolor:green;">${interfaces[j].result.status}</span>）`
                }else{
                    html+=`（状态：<span style="dolor:red;">${interfaces[j].result.status}</span>）`
                }
                
            } 
            if (interfaces[j].runTime) {
                html+=`（耗时<span style="dolor:green;">${interfaces[j].runTime}</span>秒）`
            } 
...
exports.runTestCode2=runTestCode2;
