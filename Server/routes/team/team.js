...
function Team() {
    ...
    this.info2=async (req,res)=> {
        try
        {
            ...
            let ret=await (project.findAsync(query,"name dis owner users createdAt",{
                sort:"name"
            }));
            //console.log(ret)
            for(let objPro of ret){
               
				let joinUsers=objPro.users.map(function (obj) {
					return obj.user.toString()
				});;
				if(userId.toString()==objPro.owner.toString()){
					objPro._doc.own=1;
					objPro._doc.role=0; 
				}
				...
				{
                    objPro._doc.own=0;
					objPro._doc.role=objPro.users[joinUsers.indexOf(userId.toString())].role;
				}
				else
				{
					objPro._doc.own=0;
                ...
                objPro._doc.userCount=objPro.users.length+1;
                delete objPro._doc.users;
                objPro._doc.interfaceCount=await (interface.countAsync({
                    project:objPro._id
                }))
    ...
    this.info3=async (req,res)=> {
        try
        {
            ...
            ret=await (project.findAsync(query,"name dis owner users createdAt",{
                sort:"name"
            }));
            //console.log(ret)
            for(let objPro of ret){
    ...
    this.projectUser=async (req,res)=> {
        try
        {
            ...
            // console.log("team/projectUser/arrUser")
            // console.log(arrUser)
            for(let obj of arrUser)
            {
                for(let obj1 of obj.users)
                {
                    // console.log("team/projectUser/arrUser-user")
                    // console.log(obj1)
    ...
    this.projectUser2=async (req,res)=> {
        try
        {
            let objPro=await (project.findOneAsync({
                _id:req.clientParam.project
            ...
            // console.log("team/projectUser2/objPro")
            // console.log(objPro)
            if(!objPro)
            {
                util.throw(e.projectNotFound,"项目不存在");
            ...
            let projOwner=await (user.findOneAsync({
                 _id:objPro.owner
             },"name photo"))
            arr.unshift({
                user:projOwner,
    ...
    this.removeProject2=async (req,res)=> {
        try
        {
            let obj=await (project.findOneAndUpdateAsync({
                _id:req.clientParam.project
    ...
    this.allList=async function (req,res) {
        try
        {
            let obj=[],ret=[];
            let userId=req.userInfo._id;
            
            // console.log('team>allList>userId')
            // console.log(userId)
            let arr=await (team.findAsync({
                //owner:req.userInfo._id
            },"name",{
            ...
            // console.log('team>allList>arr add ')
            // console.log(arr)
            for(let objTeam of arr)
            {
                ...
                arrProject=await (project.findAsync(query,"name users owner",{
                    sort:"name"
                }));
