...
function User() {
    ...
    this.allList=async (req,res)=>{
        try
        {
            let obj=await (user.findAsync({},"name email",{
                sort:"name"
            }))
            util.ok(res,obj,"ok");
        }
    ...
    this.quickRegister=async (req,res)=>{
        try
        {
            ...
            let emailArr=req.clientParam.emails.split(";");
            let users=[]
            for (let email of emailArr) {
                let userinfo={}
                userinfo.email=email
                let name=email.split("@")[0].replace(/^\s+|\s+$/g,''); 
                userinfo.name=name
                userinfo.password=name
                userinfo.question="your name"
                userinfo.answer=name
                users.push(userinfo)
                let ret=await (user.findOneAsync({
                    name:name
                }));
                if(ret)
                {
                    ...
                    userinfo=await (user.createAsync(userinfo));
                    delete userinfo._doc.password;
                }
            }
            
            util.ok(res,users,"ok");
