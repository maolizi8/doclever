...
function Project() {
    ...
    this.interfaceList2=async (req,res)=> {
        ...
        {
            let obj={}
            let proj=await (interface.findOneAsync({
                _id:req.clientParam.id
            },"project"));
            if (!proj) {
            ...
            let projBaseUrl=await (project.findOneAsync({
                _id:proj.project
            },"baseUrls"));
            if (!projBaseUrl) {
                util.throw(e.projectNotFound, "项目不存在");
            ...
            (obj._doc?obj._doc:obj).baseUrl=projBaseUrl.baseUrls;
            
            // console.log("interfaceList2>obj")
            // console.log(obj)
        ...
        try
        {
            ...
            if(obj.tags)
            {
                for(let o of obj.tags)
                {
                    ...
                    if(o.description)
                    {
                        objRemark[o.name]=o.description;
                    }
                }
            }
    ...
    this.projectGroup=async (req,res)=> {
        try
        {
            let projectArr=await (project.findAsync({},"name",{
                sort:"name"
            }));

            for (let projObj of projectArr) {
                let groupArr=await (group.findAsync({
                    project:projObj._id
                },"name",{
                    sort:"name"
                }));
                (projObj._doc?projObj._doc:projObj).data=groupArr;
            }
