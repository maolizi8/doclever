...
function Interface() {
    ...
    this.validateUser = async (req)=> {
        ...
        if (req.clientParam.id) {
            ...
            //console.log('server/routes/interface/interface.js')
            //console.log(obj)
            pro = obj.project;
        }
        else {
    ...
    this.create=async (req, res)=> {
        try {
            ...
            if (req.clientParam.id) {
                ...
                if (req.headers["docleversnapshot"]) {
                    update.snapshot = decodeURIComponent(req.headers["docleversnapshotdis"].replace(/\+/g, '%20'));
                }
    ...
    this.transferProject=async (req, res)=> {
        try {
            
            await (interface.findOneAndUpdateAsync({
                _id: req.clientParam.id
            }, {
                project:req.clientParam.project,
                group:req.clientParam.group
    ...
    this.projectGroup=async (req, res)=> {
        try {
            
            let interInfo=await (interface.findOneAsync({
                _id: req.clientParam.id
            },"project group"))
            
            util.ok(res,interInfo,"ok");
        }
        catch (err) {
            util.catch(res, err);
        }
        ...
        try
        {
            ...
            for(let objTeam of arrTeam)
            {
                ...
                for(let objPro of arrProject)
                {
                    ...
                    (objPro._doc?objPro._doc:objPro).data=arrGroup;
                    for(let objGroup of arrGroup)
                    {
                        ...
                        (objGroup._doc?objGroup._doc:objGroup).data=arrInterface;
                    }
                }
            }
            util.ok(res,arrTeam,"ok")
        }
        catch (err)
        {
            util.catch(res,err);
