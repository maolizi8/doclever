...
var userGroup=require("../../model/userGroupModel")
...
function Common() {
    ...
    this.userList=async (type,key,page)=>{
        ...
        // 原来的0-今日注册，3-全部用户，调换一下：3-今日注册，0-全部用户
        //0-所有用户/用户搜索,   1-今天登录,   2-登录最多, 3-今日注册, 
        if(type==0)
        {
            /* query={
                createdAt:{
        ...
        arr=await (user.findAsync(query,"name password photo createdAt lastLoginDate loginCount state",{
            sort:sort,
            skip:10*page,
            limit:10
        }))
    ...
    this.userGroupList=async (key)=>{
        
        let arr=[];
        let query={
            name:new RegExp(key?key:"","i")
        };
        //obj.total=await (userGroup.countAsync());
        
        arr=await (userGroup.findAsync(query,null,{
            sort:'name'
        }))

        return arr;
    }//gql add
    this.saveUserGroupList=async (id,name,dis,users)=>{
        ...
        if (id) {
            obj=await (userGroup.findOneAndUpdateAsync({
                _id:id
            },query,{
                new:false
            }));
        } else {
            obj=await (userGroup.createAsync(query));
        }
