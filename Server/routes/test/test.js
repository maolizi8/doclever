...
function Test() {
    ...
    this.copyTest=async (req,res)=> {
        try
        {
            let obj={};

            let originTest=await (test.findOneAsync({
                _id:req.clientParam.id
            }))
            ...
            obj={
                name:originTest.name+'-copy',
                project:originTest.project,
                module:originTest.module,
                group:originTest.group,
                remark:originTest.remark,
                owner:req.clientParam.user,
                editor:req.clientParam.user,
                status:originTest.status,
                code:originTest.code,
                ui:originTest.ui,
                //output:originTest.output,
                id:uuid(),
                user:req.clientParam.user
            }
            
            let ret=await (req.testModel.createAsync(obj));
    ...
    this.info2=async (req,res)=> {
        try
        {
			let userId=req.userInfo._id;
            let ret={};
			
            let projObj=await (testProject.findOneAsync({
                _id:req.clientParam.project
            },null,{
                populate:{
                    path:"users",
            ...
            projObj=await (testProject.populateAsync(projObj,{
                path:"owner",
                select:"name photo"
            }))
            if(req.clientParam.only)
            {
                util.ok(res,projObj,"ok");
                return;
            }
            ret.info=projObj;
			
			let joinUsers=projObj.users.map(function (obj) {
					return obj._id.toString();
				});
			if(userId.toString()==projObj.owner._id.toString()){
				ret.user=userId;
				ret.editable=1;
			}
			else if(joinUsers.indexOf(userId.toString())>-1)
			{
				ret.user=userId;
				ret.editable=1;
			}
    ...
    this.projectList2=async (req,res)=>{
        try
        {
			let userId=req.userInfo._id;
			
            let arr=await (testProject.findAsync({},null,{
                sort:"name"
            }))
			
            for(let obj of arr)
            {
				//console.log("test.js>projectList2>obj")
				//console.log(obj)
				let joinUsers=obj.users.map(function (obj) {
					return obj._id.toString();
				});
				
				if(userId.toString()==obj.owner.toString()){
					obj._doc.own=1;
					obj._doc.role=0;
				}
				else if(joinUsers.indexOf(userId.toString())>-1)
    ...
    this.removeProject=async (req,res)=>{
        try
        {
            ...
            let modules=testModule.findAsync({
                project:req.clientParam.project
            },"name")

            for (let obj of modules) {
                //console.log("module.id:"+obj._id)
	...
	this.filterList2=async (req,res)=>{
        try
        {
            let query={
                name:new RegExp(req.clientParam.name,"i")
    ...
    this.interfaceInfo=async (req,res)=>{
        try
        {
            ...
            // console.log('Server>routes>test>test.js>this.interfaceInfo-req.clientParam')
            // console.log(req.clientParam)
            // console.log('Server>routes>test>test.js>this.interfaceInfo-req.clientParam.interface')
            // console.log(req.clientParam.interface)
            ...
            util.ok(res,ret,"ok");
            // console.log('Server>routes>test>test.js>this.interfaceInfo-ret')
            // console.log(ret)
    ...
    this.copyCollection=async (req,res)=>{
        try
        {
            
            let originCollection=await (testCollection.findOneAsync({
                _id:req.clientParam.id
            }));
            if (!originCollection) {
                util.throw(e.testCollectionNotFound,"要复制的集合不存在！");
            }
            let query={
                name:originCollection.name+"-copy",
                project:originCollection.project,
                tests:originCollection.tests,
                user:req.clientParam.user
            };
            let obj=await (testCollection.createAsync(query))
    ...
    this.collectionOfTest=async (req,res)=>{
        try
        {
            let projList=await (testProject.findAsync({},"name",{
                sort:"name"
            }))
            for(let obj of projList){
                let arr=await (testCollection.findAsync({
                    project:obj._id
                },"name",{
                    sort:"name"
                }));
                (obj._doc?obj._doc:obj).data=arr;
            }
            
            util.ok(res,projList,"ok");
        }
        catch (err)
        {
            util.catch(res,err);
    ...
    this.updateInterfaceProject=async (req,res)=>{
        try
        {
            let testData=[]
            // let testArr=await test.findAsync({},"ui");
            // for (let test of testArr) {
            //     //console.log(test)
            //     for (let data of test.ui) {
            //         if (data.type=="interface") {
            //             let interInfo=JSON.parse(data.data)
    ...
    this.migrateTestTest=async (req,res)=>{
        try
        {
            let testData=[]
    ...
    this.migrateTestGroup=async (req,res)=>{
        try
        {
            let ret={}
            let updateGroup=await (testGroup.findOneAndUpdateAsync({
                _id:req.clientParam.group
            },{
                module:req.clientParam.module
            },{
            ...
            let updateTest=await (testGroup.updateAsync({
                group:req.clientParam.group
            },{
                module:req.clientParam.module,
                project:req.clientParam.project
