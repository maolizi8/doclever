<template>
    <el-row class="row" style="font-size:14px;position: fixed;width: auto;z-index: 10000;border-radius:3px;box-shadow:0 2px 12px 0 rgba(0,0,0,.1);border:1px solid #e4e7ed" :menuIndex="val.length+1">
        <el-col class="col" style="width:150px;background: white;height: 200px;overflow-y: auto" ref="menu" :menuIndex="val.length+1">
            <el-row class="row" v-for="(item,index) in arr" :key="index" style="height: 35px;line-height: 35px" @mouseenter.native="enter(item)" @mouseleave.native="leave(item,$event)" :style="{backgroundColor:item.select?'#F3F3F3':''}" :menuIndex="val.length+1" @click.native="select(item)">
        ...
        <template v-for="(item,index) in arr">
            <multimenu v-if="item.data && item.data.length>0 && item.select" :val="val.concat(item.value)" :source="item.data" style="margin-left:150px" :key="String(index)" @click="click" @level="level"></multimenu>
        </template>
    </el-row>
</template>
