...
helper.runTest=async function (obj,global,test,root,opt,id,testid) {
    console.log("<helper.js--runTest>")
    console.log(root)
    ...
    console.log("client>web>common>js>helpler.js>1808-obj")
    console.log(obj)
    ...
    // console.log("client>web>common>js>helpler.js>1825-globalVar")
    ...
    if(indexSlash>-1)
    {
        ...
        baseurl=baseUrlTemp;
        // console.log('help.js>baseUrlTemp、pathTemp')
    ...
    // console.log('help.js>path before handleGlobalVar')
    // console.log(path)
    path=helper.handleGlobalVar(path,globalVar);
    // console.log('help.js>path after handleGlobalVar')
    // console.log(path)
    if(path.substr(0,2)=="//")
    {
        path=path.substr(1);
    ...
    return func.then(function (result) {
        ...
        if (res.req.info.cookie) {
            ...
            var newstr=noquotes.replace(/=/g,"\":\""); 
            var stringObj= "{\"" +newstr.replace(/;(\s*)/g,"\",\"") + "\"}";
            // console.log("stringObj")
            // console.log(stringObj)
            res.cookies=JSON.parse(stringObj)
        }else{
            res.cookies={}
        }
...
helper.runTestCode=async function (code,test,global,opt,root,argv,mode,__id,level,testid) {
    var Base64=BASE64.encoder,MD5=CryptoJS.MD5,SHA1=CryptoJS.SHA1,SHA256=CryptoJS.SHA256,SHA512=CryptoJS.SHA512,SHA3=CryptoJS.SHA3,RIPEMD160=CryptoJS.RIPEMD160,AES=CryptoJS.AES.encrypt,TripleDES=CryptoJS.TripleDES.encrypt,DES=CryptoJS.DES.encrypt,Rabbit=CryptoJS.Rabbit.encrypt,RC4=CryptoJS.RC4.encrypt,RC4Drop=CryptoJS.RC4Drop.encrypt;
    
    console.log('helper.js>runTestCode>')
    console.log(root)
    ...
    var arrNode=[];
    // console.log('helper.js>runTestCode>ele')
    // console.log(ele)
    // console.log("---for---")
    for(var i=0;i<arr.length;i++)
    {
        ...
        var text;
        // console.log('helper.js>runTestCode>obj type')
        // console.log(obj)
        // console.log(type)
        if(type=="1")
        {
            ...
            // console.log('helper.js>runTestCode>o')
            // console.log(o)
            var query={
            ...
            //console.log('helper.js>runTestCode>/test/interfaceproject-root')
            // console.log(root)
            var bFind=false;
            ...
            if(!bFind)
            {
                try
                {
                    objInfo=await net.get("/test/interfaceproject",query).then(function (data) {
                        // console.log('helper.js>runTestCode>/test/interfaceproject-data')
                        // console.log(data)
                        if(data.code==200)
                        {
                            return data.data;
        ...
        else if(type=="2")
        {
            ...
            // console.log('helper.js>runTestCode>type=2,collection run>testMode')
            ...
            try
            {
                ...
                console.log('helper.js>runTestCode>type=2,collection run>queryTest')
                console.log(queryTest)
                testObj=await net.get("/test/test",queryTest).then(function (data) {
                    console.log('helper.js>runTestCode>type=2,collection run>queryTest>data')
                    console.log(data)
                    if(data.code==200)
                    {
                        return data.data
            ...
            {   
                console.log('helper.js>runTestCode>type=2,collection run>queryTest>catch-err')
                console.log(err)
                testObj={
                    code:"",
                    ui:[],
            ...
            delete testObj.code;
            delete testObj.ui;
            text="(function () {var argv=Array.prototype.slice.call(arguments);return helper.runTestCode('"+code1+"',"+JSON.stringify(testObj)+",global,opt,root,argv,'"+testMode+"',"+(level==0?objId:undefined)+","+(level+1)+",testid)})"
        }
        else
        {
    ...
    if(__id!=undefined)
    {   
        console.log('helper.js>runTestCode>testRunStatus,testStart')
    ...
    var ret=eval("(async function () {"+ele.innerText+"})()").then(function (ret) {
        ...
        // console.log('helper.js>runTestCode>temp')
        // console.log(temp)
        if(temp===undefined)
        {
            obj.pass=undefined;
        ...
        // console.log('helper.js>runTestCode>obj')
        // console.log(obj)
        return obj;
    });
    // console.log('helper.js>runTestCode>ret')
    // console.log(ret)
    return ret;
}
