...
helper.runBefore2=function (code,requestObj,query,header,body,param) {
    var Base64=BASE64.encoder,MD5=CryptoJS.MD5,SHA1=CryptoJS.SHA1,SHA256=CryptoJS.SHA256,SHA512=CryptoJS.SHA512,SHA3=CryptoJS.SHA3,RIPEMD160=CryptoJS.RIPEMD160,AES=CryptoJS.AES.encrypt,TripleDES=CryptoJS.TripleDES.encrypt,DES=CryptoJS.DES.encrypt,Rabbit=CryptoJS.Rabbit.encrypt,RC4=CryptoJS.RC4.encrypt,RC4Drop=CryptoJS.RC4Drop.encrypt;
    try
    {
        console.log("help.js>runBefore2>code")
        console.log(code)
        if(code)
        {
            eval(code);
...
helper.convertToCode=function (data) {
    ...
    data.forEach(function (obj) {
        ...
        {
			console.log("help.js>convertToCode>obj")
			console.log(obj)
            var argv="{";
        ...
        {
            console.log('helper.js>assert>obj.data')
...
helper.getTestUIArgvList=function (arrUI,index) {
    console.log('help.js>getTestUIArgvList>arrUI')
    console.log(arrUI)
    console.log(index)
    ...
    arrUI.forEach(function (obj) {
        ...
        else
        {
            ...
            console.log('help.js>getTestUIArgvList>objInter.outInfo')
            console.log(objInter)
            console.log(objInter.outInfo)
...
// gql add
helper.runInterface=async function (obj,global,test,root,opt,id,testid,source) {
    console.log("<helper.js--runInterface>")
    
	console.log("<helper.js--runInterface>obj")
	console.log(obj)
	
	console.log("<helper.js--runInterface>opt")
	console.log(opt)

    root.output+="[开始运行接口]："+obj.name+"<br>"
    if(id!=undefined && source=='test')
    {
        // console.log("<helper.js--runInterface>testRunStatus--interfaceStart")
        // console.log(id)
    ...
    // console.log("helper.js--runInterface>globalVar")
    ...
    if(!baseurl)
    {
        ...
        //return {};
        root.output+="[结束运行接口]："+obj.name+"<span style='color:red'>(请检查接口信息)</span><br>"
        //root.output+="<br>";
        res={
            status:0,
            header:{},
        ...
        //window.vueObj.$store.state.event.$emit("testRunStatus","testUnknown",id,res);//testUnknown,interfaceFail
        if(id!=undefined && source=='test')
        {
            window.vueObj.$store.state.event.$emit("testRunStatus","interfaceFail",id,res);//testUnknown,interfaceFail
        }
        return res
    ...
    // console.log('helper.js--runInterface>obj.header')
    ...
    obj.header.forEach(function (obj) {
        // console.log('helper.js--runInterface>for>obj>$.inArr(obj.name,arrHeaders)')
        // console.log(obj.name)
        // console.log($.inArr(obj.name,arrHeaders))
	...
	// console.log("help.js>runInterface>after pullInject>header,objHeaders")
    // console.log(header)
            ...
            for(var i=0;i<obj.bodyParam.length;i++)
            {
                ...
                else if(obj1.type==1)
                {
                    ...
                    var file=await (new Promise(function (resolve,reject) {
                        ...
                        var child=$.showBox(window.vueObj,require("../../controller/testproj/test/component/testUploadFile.vue"),{
                            name:name,
                            url:path,
                            keyName:obj1.name,
                            remark:obj1.remark
    ...
    if(obj.before.mode==0)
    {
        if(global.before)
        {
            ...
            helper.runBefore2(global.before,requestObj,query,header,body,param)
        }
        //helper.runBefore(obj.before.code,baseurl,path,method,query,header,body,param)
        helper.runBefore2(obj.before.code,requestObj,query,header,body,param)
    }
    else
    {
        //helper.runBefore(obj.before.code,baseurl,path,method,query,header,body,param)
	...
	console.log("help.js>runInterface>after runBefore2>requestObj")
    console.log(requestObj)
	
	console.log("help.js>runInterface>obj.pullInject:"+obj.pullInject)
	
    if(!obj.pullInject)
    {
		if(opt && opt.requestObj)
        {
            /* for(var key in opt.requestObj)
            {
                if($.inArr(key,requestObj))
                {
                    requestObj[key]=opt.requestObj[key];
                }
            } */
	...
	console.log("help.js>runInterface>before path add query>requestObj")
    console.log(requestObj)
	...
	console.log("help.js>runInterface>before header>header,objHeaders")
    console.log(header)
	console.log(objHeaders)
	...
	console.log("help.js>runInterface>after header>header,objHeaders")
    console.log(header)
	console.log(objHeaders)
    // console.log("help.js>runInterface>header")
    // console.log(header)
    // console.log("help.js>runInterface>objHeaders")
    // console.log(objHeaders)
    ...
    console.log("helper.js>net.post>body,header,bNet")
        console.log(body)
        console.log(header)
        console.log(bNet)
    ...
    return func.then(function (result) {
        console.log("helper.js>runInterface>result.data")
        console.log(result.data)
        console.log("helper.js>runInterface>result.header")
        ...
        res.cookieStr=res.req.info.cookie;

        if (res.req.info.cookie) {
            console.log("res.req.info.cookie:")
            console.log(res.req.info.cookie)
            var noblank=res.req.info.cookie.replace(/(^\s*)|(\s*$)/g, "");
            var noquotes=noblank.replace(/\"\"/g, "");
            noquotes=noblank.replace(/\"/g, "");
            var doubleEqual=noquotes.replace(/==/g,"%3D%3D"); 
            var newstr=doubleEqual.replace(/=/g,"\":\""); 
            var stringObj= "{\"" +newstr.replace(/;(\s*)/g,"\",\"") + "\"}";
            console.log("stringObj")
            console.log(stringObj)
            try {
                res.cookieDict=JSON.parse(stringObj)
                console.log("res.cookieDict")
                console.log(res.cookieDict)
            } catch (error) {
                console.log("!!!!!!!!!result cookie transfer to cookie failed")
                res.cookieDict={}
            }
        }else{
            res.cookieDict={}
        ...
        if(id!=undefined && source=='test')
        {
            console.log("test>> update interface status")
        ...
        root.output+="<span style='color:red'>[ERROR]："+err.message+"</span><br>";
...
helper.runTestCode2=async function (code,test,global,opt,root,argv,mode,__id,level,testid,source) {
    var Base64=BASE64.encoder,MD5=CryptoJS.MD5,SHA1=CryptoJS.SHA1,SHA256=CryptoJS.SHA256,SHA512=CryptoJS.SHA512,SHA3=CryptoJS.SHA3,RIPEMD160=CryptoJS.RIPEMD160,AES=CryptoJS.AES.encrypt,TripleDES=CryptoJS.TripleDES.encrypt,DES=CryptoJS.DES.encrypt,Rabbit=CryptoJS.Rabbit.encrypt,RC4=CryptoJS.RC4.encrypt,RC4Drop=CryptoJS.RC4Drop.encrypt;
    
    console.log('helper.js>runTestCode2>')
    console.log(root)
	
	console.log("<helper.js--runTestCode2>opt")
    console.log(opt)
    console.log("<helper.js--runTestCode2>level")
	console.log(level)
	console.log("<helper.js--runTestCode2>argv")
	console.log(argv)
    ...
    function __assert(val,id,title) {
        ...
        console.log("<helper.js--runTestCode2>__assert>val,id,title")
        console.log(val)
        console.log(id)
        console.log(title)
    }
    ...
    function input(title,data) {
        return new Promise(function (resolve,reject) {
            ...
            var child=$.showBox(window.vueObj,require("../../controller/testproj/test/component/testRunInput.vue"),{
                title:title,
                data:data,
                name:test.name
            });
    ...
    for(var i=0;i<arr.length;i++)
    {
        ...
        if(type=="1")
        {
            ...
            try {
                ...
                //replace_o=replace_o.replace(/'/g,"$#39;")
            ...
            try
            {
                await net.get("/interface/projectgroup",{
                    id:o._id
                }).then(function (data) {
                    if(data.code==200)
                    {
                        console.log('helper.js>runTestCode2>update query.project')
                        query.project=data.data.project;
                    }
                    else
                    {
                        $.notify(data.msg,0);
            ...
            //console.log('helper.js>runTestCode2>/test/interfaceproject-root')
            // console.log(root)
            var bFind=false;
            ...
            if(!bFind)
            {
                try
                {
                    objInfo=await net.get("/test/interfaceproject",query).then(function (data) {
                        // console.log('helper.js>runTestCode2>/test/interfaceproject-data')
                        // console.log(data)
                        if(data.code==200)
                        {
                            return data.data;
			...
			// console.log('helper.js>runTestCode2>before runInterface>obj')
            // console.log(obj)
        ...
        else if(type=="2")
        {
            ...
            // console.log('helper.js>runTestCode2>type=2,collection run>testMode')
            ...
            try
            {
                ...
                if(obj.length!=24)
                {
                    ...
                    queryTest.project=testid
                }
                console.log('helper.js>runTestCode2>type=2,collection run>queryTest')
                console.log(queryTest)
                testObj=await net.get("/test/test",queryTest).then(function (data) {
                    console.log('helper.js>runTestCode2>type=2,collection run>queryTest>data')
                    console.log(data)
                    if(data.code==200)
                    {
                        return data.data
            ...
            {   
                console.log('helper.js>runTestCode2>type=2,collection run>queryTest>catch-err')
                console.log(err)
                testObj={
                    code:"",
                    ui:[],
    ...
    if (level>1) {
        root.output+="<br><div style='background-color: #ececec'>[开始执行(内嵌)用例]："+test.name+"("+(mode=="code"?"代码模式":"UI模式")+")<br>";
    }
    
    var startTime=Date.now();
    var startOutputIndex=root.output.length;

    console.log("client>web>common>js>helpler.js>__id")
    console.log(__id)
    console.log("client>web>common>js>helpler.js>level")
    console.log(level)
    if(__id!=undefined)
    {   
        console.log('helper.js>runTestCode2>testRunStatus,testStart')
        window.vueObj.$store.state.event.$emit("testRunStatus","testStart",__id);
    }

    // if(source=='test' && level==2)
    // {   
    //     console.log('helper.js>runTestCode2>testRunStatus,testStart')
    //     window.vueObj.$store.state.event.$emit("testRunStatus","testStart",__id);
    // }
    ...
    var ret=eval("(async function () {"+ele.innerText+"})()").then(function (ret) {
        ...
        if(temp===undefined)
        {
            ...
            // }
            if (level==1) {
                root.output+="[用例执行结束]："+test.name+"(未判定)<br>";
            }
            if (level>1) {
                root.output+="[(内嵌)用例执行结束]："+test.name+"(未判定)<br>";
            }
        }
        else if(Boolean(temp)==true)
        {
            ...
            // }
            if (level==1) {
                root.output+="[用例执行结束]："+test.name+"(<span style='color:green'>已通过</span>)<br>";
            }
            if (level>1) {
                root.output+="[(内嵌)用例执行结束]："+test.name+"(<span style='color:green'>已通过</span>)<br>";
            }
    ...
    // console.log('helper.js>runTestCode2>ret')
    // console.log(ret)
    return ret;
}
