...
module.exports={
    ...
    getters:{
        testProjInfo:function(){
			console.log("testproj>test>getters>testProjInfo")
            if (context.state.testid) {
                return Promise.all([
                    context.dispatch("test/init",{project:context.state.testid})
                ]).then(function () {
        ...
        testid:function(state,getters,rootState){
            console.log('testproj>test>store.js>getters>testid')
            console.log(rootState.testid)
            return rootState.testid;
        },//geqiuli add
    ...
    mutations:{
        init:function (state,data) {
            console.log('testproj>test>store.js>mutations>init-data')
            console.log(data)
			...
			state.editable=data?data.info.editable:0;//
    ...
    actions:{
        init:function (context) {
            console.log('testproj>test>store.js>actions>init-data')
            
            //context.commit("init");
			console.log("getters.testid:"+context.getters.testid)
			console.log("state.testid:"+context.state.testid)
            net.get("/test/interfacelist",{}).then(function (dt) {
                if(dt.code==200)
                {   
                    //console.log("testproj>test>store.js-init-interfacelist-dt")
                    //console.log(dt)
			...
			return net.get("/test/info2",{
                project:context.getters.testid,
                user:session.get("id")
			}).then(function (dt) {
				...
				if(dt.code==200)
				{
					...
					// console.log("testproj>test>store.js-/test/info-init-dt")
					// console.log(dt)
					context.state.testprojInfo=dt.data.info;
					context.state.editable=dt.data.editable;
					//context.state.data.push(dt.data.testList);
					
				}
				//console.log("testproj>test>store.js-/test/info-init-dt-return")
				// console.log(dt)
				return dt;
			});
        },
        ...
        urlList:function (context) {
            return net.get("/testproj/urllist",{
                //id:session.get("projectId")
                id:context.getters.testid
            }).then(function (data) {
                return data;
            })
        },
        ...
        copyTest:function (context,info) {
            console.log('store.js>copyTest>info')
            console.log(info)
            var query={
                id:info.data._id,
                user:context.state.selUser
            ...
            return net.post("/test/copytest",query).then(function (data) {
                if(data.code==200)
                {
                    //context.state.selTest=data.data;
                    console.log('store.js>before copyTest>context.state.data')
                    ...
                    for(var i=0;i<context.state.data.length;i++)
                    {
                        ...
                        for(var j=0;j<obj.data.length;j++)
                        {
                            ...
                            for(var k=0;k<obj1.data.length;k++)
                            {
                                ...
                                {
                                    bFind1=true;
                                    obj1.data.splice(k+1,0,data.data);
                                    break;
                                }
                            }
                            if(bFind1)
                    ...
                    console.log('store.js>after copyTest>context.state.data')
                    console.log(context.state.data)
                }else{
					throw data.msg;
				}
        ...
        info:function (context,node) {
            //console.log("testproj>test>store.js>info")
            return net.get("/test/test",{
                id:node.data._id
            }).then(function (data) {
                // console.log("testproj>test>store.js-info-data")
                // console.log(data)
                if(data.code==200)
                {
                    context.state.selTest=data.data;
        ...
        save:function (context,data) {
            ...
            return net.post("/test/test",obj).then(function (data) {
                if(data.code==200)
                {
                    if(context.state.selNode.data.group._id!=data.data.group._id)
                    {
        ...
        showInterface:function (context,obj) {
            ...
            return net.get("/test/interface",{
                ...
                console.log('testproj>test>store.js>showInterface>data')
                console.log(data)
                if(data.code==200)
                {
        ...
        addCollection:function (context,name) {
			//console.log("addCollection>testid:"+context.getters.testid)
            ...
            return net.post("/test/collection",{
                ...
                if(data.code==200)
                {
                    context.state.collection.push(data.data);
                }else{
					throw data.msg;
				}
                return data;
        ...
        copyCollection:function (context,info) {
			//console.log("copyCollection>testid:"+context.getters.testid)
			
            return net.post("/test/copycollection",{
                id:info._id,
                user:context.state.selUser
            }).then(function (data) {
                if(data.code==200)
                {
                    var cFind=false;
                    for(var k=0;k<context.state.collection.length;k++)
                    {
                        var obj=context.state.collection[k];
                        if(obj._id==info._id)
                        {
                            cFind=true;
                            context.state.collection.splice(k+1,0,data.data);
                            break;
                        }
                    }
                    if (!cFind) {
        ...
        saveCollection:function (context,data) {
            ...
            data.project=context.getters.testid;
        ...
        changeExample:function (context,obj) {
            ...
            return pro.then(function (data) {
                if(data.code==200)
                {
                    ...
                    console.log("testproj>test>store.js>changeExample>")
                    if(obj.id)
                    {
                        console.log("testproj>test>store.js>changeExample>data.data.name")
