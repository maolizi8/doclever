<template>
    <el-row class="row">
        <el-row class="row" style="padding:0 0 0 20px;height: 36px;line-height: 36px">
            ...
            <template v-for="(item,index) in arr">
                <tr :style="{textAlign: 'center',verticalAlign: 'middle',backgroundColor:item.enable?'white':'lightgray'}" :key="index" style="height: 36px;line-height: 36px;">
                    ...
                    <td style="width: 10%;text-align: center;vertical-align: middle;">
                        <el-select size="small" v-model="item.type" style="width: 100%" :disabled="true">
                            <el-option :value="0" label="文本"></el-option>
                            <el-option :value="1" label="文件"></el-option>
                        </el-select>
                    </td>
                    <td style="width: 35%;text-align: center;vertical-align: middle;">
                        <div  style="width: 100%;display: inline-block;" v-if="item.type==0 && item.value && (item.value.data.length>0 || item.value.status)">
                            <el-autocomplete size="small" class="inline-input" v-model="item.selValue" :fetch-suggestions="querySearch" placeholder="选择或者填入你的值" @mouseenter.native="focus(item)" :disabled="!item.enable || interfaceObj.example" custom style="width:100%" popper-class="my-autocomplete">
                        ...
                        <el-input size="small" style="width: 100%;" placeholder="请填写值" v-model="item.selValue" v-else-if="item.type==0" custom :disabled="!item.enable || interfaceObj.example"></el-input>
                    </td>
                    <!-- <td style="width: 5%;" >
                    ...
                    <td style="width: 20%;line-height: normal;overflow-y: auto">
                        {{item.remark?item.remark:"无备注"}}
                    </td>
                    <td style="width: 10%;">
                        <el-button type="text" size="mini"  style="font-size: 13px" @click="encrypt(item)" v-if="item.type==0" :disabled="interfaceObj.example">{{(item.encrypt && item.encrypt.type)?item.encrypt.type:"未加密"}}</el-button>
                    </td>
                    <td style="width: 5%">
                        <el-button type="text" size="mini" style="font-size: 15px;" @click="toggleEnable(item,index)" :disabled="interfaceObj.example"><span :class="item.enable?'fa fa-eye':'fa fa-eye-slash'" :title="item.enable?'运行时包含此字段':'运行时不包含此字段'"></span></el-button>
                    </td>
                </tr>
            </template>
        </table>
        <el-row class="row" style="padding: 0 0 0 20px;" v-show="info.type==1 && info.rawType==2">
            <testbodyjson :source="info.rawJSON" :status="status" :interface="interfaceObj"></testbodyjson>
        </el-row>
        <el-row class="row" style="padding: 0 0 0 20px;" v-show="info.type==1 && info.rawType!=2">
            <el-row class="row" style="height: 36px;line-height: 36px;margin: 0;padding: 0" v-if="info.rawType==0">
                <el-col class="col" :span="3" style="text-align: center" v-if="info.rawType==0">
                    加密类型
                </el-col>
                <el-col class="col" :span="5" style="text-align: center">
                    <el-select size="small" id="bodyRawEncryptType" v-model="interfaceObj.encrypt.type" v-if="info.rawType==0">
                ...
                <el-col class="col" :span="5" style="text-align: center" v-show="info.rawType==0 && saltShow">
                    <el-input size="small" placeholder="请填入salt的值" id="bodyRawEncryptSalt" v-model="interfaceObj.encrypt.salt"></el-input>
                </el-col>
            </el-row>
...
<script>
    ...
    module.exports={
        ...
        computed:{
            saltShow:function () {
                return helper.isSalt(this.interfaceObj.encrypt.type);
            },
            arr:function () {
                return this.interfaceObj.bodyParam
            },
            info:function () {
                return this.interfaceObj.bodyInfo
            },
            rawType:{
                get:function () {
                    var type="";
                    this.interfaceObj.header.forEach(function (obj) {
