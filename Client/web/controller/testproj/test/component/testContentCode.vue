<template>
    <el-row class="row" id="testContentCode">
        <el-row class="row" style="height: 50px;line-height: 50px">
            ...
            <el-button size="mini" type="primary" style="margin-left: 20px" @click="insertInterface" v-if="type!=1">插入接口</el-button>&nbsp;&nbsp;&nbsp;
        ...
        <el-row class="row" style="min-height: 300px;border: 1px solid lightgray;margin-left: 10px;width: calc(100% - 20px)">
            <el-col class="col" style="width: 40px;background-color: lightgray;padding-top: 5px;overflow-y: hidden;" :style="{paddingBottom:5+scrollHeight+'px'}" id="testScroll">
                <el-row class="row" style="text-align: right;padding-right: 5px;line-height: 1.5;font-size: 15px" v-for="n in line" :key="n">
                    {{n}}
            ...
            <el-col class="col" style="width:calc(100% - 40px);box-sizing:padding-box;background-color: rgba(255,255,224,0.5)">
                <div class="row" style="" contenteditable="true" id="testContent" spellcheck="false" @input="changeContent($event)" @keydown="keyDown($event)" @scroll="scroll($event)" @keyup="keyUp($event)">

                </div>
            </el-col>
        </el-row>
        <el-row class="row" style="" v-if="menu && vars.length>0" id="autoMenu">
            <el-row class="row autoMenu" v-for="(item,index) in vars" :key="index" style="width: 100%;height: 30px;line-height:30px;text-align:left;padding-left: 5px;cursor: default" :style="{backgroundColor:selMenuIndex==index?'#22B5F3':'white',color:selMenuIndex==index?'white':'gray'}" @click.native="selectMenu(item)">
                {{item}}
            </el-row>
        </el-row>
    </el-row>
    ...
    module.exports = {
        ...
        methods: {
            ...
            insertInterface:function () {
                ...
                var _this=this;
                console.log('testcontentcode.vue>insertInterface>_this')
                console.log(_this)
            ...
            editInterface:function (ele) {
                console.log("testContentCode.vue>editInterface>ele")
                console.log(ele)
