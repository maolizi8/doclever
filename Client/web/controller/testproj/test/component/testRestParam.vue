    ...
    <el-row class="row">
        <table style="width: 100%" id="paramTable">
            <template v-for="item in arr">
                <tr style="text-align: center;vertical-align: middle;height: 50px;line-height: 50px" :key="item.name">
                    <td style="width: 30%">
                        {{item.name}}
                    </td>
                    ...
                    <td style="width: 30%">
                        <div  style="width: 90%;display: inline-block;" v-if="item.value && (item.value.data.length>0 || item.value.status)">
                            <el-autocomplete size="small" class="inline-input" v-model="item.selValue" :fetch-suggestions="querySearch" placeholder="选择或者填入你的值"  @mouseenter.native="focus(item)" style="width:100%" popper-class="my-autocomplete" :disabled="interfaceObj.example">
                        ...
                        <el-input size="small" v-else style="width: 90%" v-model="item.selValue" placeholder="请填写值" :disabled="interfaceObj.example"></el-input>
                    </td>
                </tr>
            </template>
        </table>
...
<script>
    module.exports={
        ...
        computed: {
            arr:function () {
                return this.interfaceObj.restParam
            }
        },
        methods:{
