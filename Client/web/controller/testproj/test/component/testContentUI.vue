    ...
    <el-row class="row">
        <el-row class="row" style="height: 50px;line-height: 50px">
            <el-dropdown trigger="hover" style="margin-left: 15px">
                <div class="el-dropdown-link">
                    <el-button size="mini" type="primary">
                        {{type==1?"接口":"接口/用例"}}&nbsp;<i class="el-icon-caret-bottom"></i>
                    </el-button>
                </div>
                <el-dropdown-menu slot="dropdown">
                    <!-- <el-dropdown-item @click.native="insertInterface">接口</el-dropdown-item> -->
                    <el-dropdown-item @click.native="insertInterface" v-if="type!=1">接口</el-dropdown-item>
            ...
            <el-dropdown trigger="hover" style="margin-left: 15px">
                <div class="el-dropdown-link">
                    <el-button size="mini" type="primary">
                        条件控制&nbsp;<i class="el-icon-caret-bottom"></i>
                    </el-button>
                </div>
                <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown trigger="hover" style="margin-left: 15px">
                <div class="el-dropdown-link">
                    <el-button size="mini" type="primary">
                        工具&nbsp;<i class="el-icon-caret-bottom"></i>
                    </el-button>
                </div>
                <el-dropdown-menu slot="dropdown">
            ...
            <a href="/html/web/resource/other/uitest.html" style="float: right;margin-right: 20px;color: #17b9e6" target="_blank">如何编写</a>
        ...
        <el-row class="row" style="margin-top: 10px;padding: 0 10px;">
            ...
            <el-row class="row" style="min-height: 100px;overflow-y: auto" id="testUIEdit" v-else>
                <el-row class="row testUILine" v-for="(item,index) in arr" :key="index" style="" draggable="true" 
                        @dragover.native="dragOver($event,item)" @dragleave.native="dragLeave($event)" 
                        @dragenter.native="dragEnter($event,item)" @drop.native="drop($event,item,index)" 
                        @dragend.native="dragEnd($event)" @dragstart.native="dragStart($event,item,index)" 
                        @mouseenter.native="$event.target.style.backgroundColor='#f3f3f3'" 
                        @mouseleave.native="$event.target.style.backgroundColor='white'">

                     <el-col class="col" :span="1">
                        {{item.id}}
                    </el-col>
                    <el-col class="col" :span="2" v-html="getTypeName(item.type,item)" :style="{textDecoration:item.modify==2?'line-through':(item.modify==1?'underline':'none'),textDecorationColor:item.modify==1?'red':'black'}">
                    </el-col>
                    <el-col class="col" :span="8" style="overflow:hidden;text-overflow:ellipsis;">
                        {{item.name}}
                    </el-col>
                    <el-col class="col" :span="8"  style="overflow: hidden;text-overflow:ellipsis;">
                        <el-tooltip class="item" effect="dark" :content="item.data" placement="bottom" v-if="showData.indexOf(item.type)>-1">
                            <span>{{item.data}}</span>
                        </el-tooltip>
                        <span v-else-if="item.type=='return'">
                            {{item.argv}}
                        </span>
                        <span v-else>
                            &nbsp;
                   ...
                   <el-col class="col" :span="1">
                    ...
                    <el-col class="col" :span="4" style="text-align:right">
...
<style>
    .testUILine{
        height: 36px;line-height: 36px;text-align: center;cursor: move;
...
/**<el-dropdown-item @click.native="insertBaseUrl">设置BaseUrl</el-dropdown-item> */
    ...
    module.exports = {
        ...
        data: function () {
            return {
                showData:["ifbegin","elseif","var","log","input","baseurl","assert"],//,"return"
                testid:this.testid,
        ...
        computed:{
            ...
            arr:function () {
                console.log("testContenUI.vue>arr(this.test.ui)")
                console.log(this.test.ui)
                return this.test.ui;
            }
        },
        watch:{
        ...
        methods: {
                ...
                this.arr.forEach(function (obj) {
                    if(obj.type=="interface")
                    {
                        ...
                        net.get("/test/interface",{
                            ...
                            if(data.code==200)
                            {
                                ...
                                obj.projectId=data.data.project;//gql add
                            }
                            else
                            {
                                obj.modify=2;
                    ...
                    else if(obj.type=="test")
                    {
                        ...
                        net.get("/test/test",{
                            ...
                            project:this.type==2?this.$store.state.selTest.project:this.testid
                        }).then(function (data) {
                            if(data.code==200)
                            {
                                obj.modify=0;
            ...
            editInterface:function (item,index) {
                ...
                console.log('testContentUI.vue>editInterface>item')
                console.log(item)
               objInterface.projectNew=item.projectId;

                $.startHud();
                var _this=this;
                var arrPromise=[
                    net.get("/project/interface2",{
            ...
            insertTest:function () {
                $.startHud();
                var _this=this;
                console.log('testContentUI>insertTest>this.testid')
            ...
            setStatus:function (type,id,res) {
                console.log('testContentUI.vue>setStatus>type')
                console.log(type)
                console.log('testContentUI.vue>setStatus>id')
                console.log(id)
                console.log('testContentUI.vue>setStatus>res')
                console.log(res)
                ...
                console.log('testContentUI.vue>setStatus>item')
                console.log(item)
                if(item)
                {
            ...
            showInterfaceOutput:function (item) {
                console.log('================================================')
                console.log('testContentUI.vue>showInterfaceOutput>item')
                console.log(item)
                if(item.info)
                {
