    ...
    <el-row class="row" style="cursor: pointer;height: 100%">
        ...
        <table width="100%" style="border-spacing: 0">
            <template v-for="(item,index) in arr">
                <tr style="text-align: center;vertical-align: middle;cursor: move;height: 36px;line-height: 36px" :key="index">
                    <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
                        <el-col class="col" :span="2" v-if="(item.type==4 || item.type==3)">
                            <!-- @click.native="toggle(item)" <span :class="item.show?'el-icon-caret-bottom':'el-icon-caret-right'" style="color:#c7c7c7 "></span> -->
                            <span class="el-icon-caret-bottom" style="color:#c7c7c7 "></span>
                        </el-col>
                    ...
                    <td style="width: 18%">
                        <el-input size="small" type="textarea" style="width: 90%;height: 32px;line-height: 32px" resize="none" :rows="0" placeholder="请填写备注;" v-model="item.remark" :disabled="true"></el-input>
                    </td>
                    <td style="width: 30%">
                        <el-autocomplete size="small" class="inline-input" v-model="item.mock" :fetch-suggestions="querySearch" placeholder="选择或者填入你的值" @mouseenter.native="focusAuto(item)" style="width:100%" popper-class="my-autocomplete" v-if="item.value && (item.value.data.length>0 || item.value.status)" :disabled="interfaceObj.example">
                        ...
                        <el-input size="small" placeholder="请填写值" v-model="item.mock" v-else-if="item.type!=3 && item.type!=4 && (!item.value || (item.value.data.length==0 && !item.value.status))" :disabled="interfaceObj.example"></el-input>
                    </td>
                </tr>
                <!-- <tr v-if="(item.type==4 || item.type==3) && (item.data && item.data.length>0) && item.show" :key="index"> -->
                <tr v-if="(item.type==4 || item.type==3) && (item.data && item.data.length>0)" :key="index">
                    <td colspan="7" style="width: 100%;margin:0;padding: 0 ">
                        <testparambodyjson :source="item.data" :le="level+1" :status="status" :interface="interface"></testparambodyjson>
                    </td>
                </tr>
            </template>
        </table>
...
<script>
    module.exports={
        ...
        computed:{
            arr:function () {
                console.log("testBodyJSON.vue>arr(this.source)")
