<template>
    <el-row class="row">
        <el-row class="row" style="height: 35px;line-height: 35px">
            ...
            <!-- <el-button type="primary" size="mini" style="float: right;margin-top: 4px;margin-right: 5px;margin-left: 0px" @click="pollPop" v-if="editRole">
                {{info.poll?"编辑定时任务":"加入定时任务"}}
            </el-button> -->
        ...
        <el-row class="row content-body" style="">
            <el-row class="row box-shadow" style="min-height:100px;">
                ...
                <el-row class="row" style="border-bottom-left-radius: 5px;border-bottom-right-radius: 5px">
                    ...
                    <el-row class="row" style="min-height: 100px;overflow-y: auto;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px" id="collectionInfo">
                        <el-row class="row" style="height: 40px;line-height: 40px;text-align: center;cursor: move;border-bottom: 1px solid #ccc;" 
                        v-for="(item,index) in arr" :key="index" @mouseenter.native="$event.target.style.backgroundColor='#f3f3f3'" @mouseleave.native="$event.target.style.backgroundColor='white'" 
                        draggable="true" @dragover.native="dragOver($event,item)" @dragleave.native="dragLeave($event)" @dragenter.native="dragEnter($event,item)" @drop.native="drop($event,item,index)" @dragend.native="dragEnd($event)" @dragstart.native="dragStart($event,item,index)">
                            <el-col class="col" :span="6" style="overflow: hidden;text-overflow:ellipsis;">
                                <span>{{item.test.module.name}}/{{item.test.group.name}}</span>
                            </el-col>
                            <el-col class="col" :span="6" style="overflow: hidden;text-overflow:ellipsis;">
            ...
            <el-row class="row" style="min-height:100px;padding: 15px 0">
                <el-row class="row box-shadow" style="height: 100%;">
                    <el-row class="row" style="height:40px;line-height: 40px;padding-left: 10px;font-size: 14px;color: #17B9E6">
                        输出   <el-button v-if="hasrun" type="text" size="mini" @click="showDetailOutput(collectionRunInfo)">
                                    详细内容
                                </el-button>
                    </el-row>
                    <el-row class="row" style="height: 1px;background-color: lightgray"></el-row>
                    <el-row class="row" style="min-height: 100px;;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;overflow-y: auto;padding-left: 10px">
        ...
        methods: {
            ...
            save:function () {
                ...
                this.$store.dispatch("saveCollection",{
                    ...
                    output:JSON.stringify(_this.info.output)
                }).then(function (data) {
            ...
            showDetailOutput:function () {
                var _this=this;
                var child=$.showBox(_this,require("./collectionRunDetailOutput.vue"),{
                    info:_this.collectionRunInfo
                });
            ...
            setStatus:function (type,id) {
                ...
                console.log('conllectionInfo.vue>setStatus')
            ...
            //             //project:session.get("projectId"),
            //             project:_this.testid,
            //             user:_this.$store.state.selUser
            //         }),
            ...
            pollPop:function () {
                var _this=this;
                $.startHud();
                var collectionId=_this.info._id;
                var testProjId=_this.info.project;
                var arr=[
                ...
                Promise.all(arr).then(function (data) {
                    ...
                    var child=$.showBox(_this,require("../../../testpoll/component/poll.vue"),{
                        "propPoll":poll,
                        "propUser":user,
