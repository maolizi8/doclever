    ...
    <el-row class="row" style="cursor: pointer;height: 100%">
        <table width="100%" style="border-spacing: 0;border-collapse: collapse">
            <template v-for="(item,index) in arr">
                <tr style="text-align: center;vertical-align: middle;cursor: move;height: 36px;line-height: 36px" :draggable="item.drag?item.drag:'false'" @dragover="dragOver($event,item)" @dragleave="dragLeave($event,item)" @dragenter="dragEnter($event,item)" @drop="drop($event,item,index,arr)" @dragstart="dragStart($event,item,index,arr)" @dragend="dragEnd($event)">
                    <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
                        ...
                        <el-col class="col" :span="22">
                            <el-input size="small" class="param-input" :placeholder="(item.name!=null && (level!=0 || type!=1))?'请填写名称':'该字段没有名称'" v-model.trim="item.name" @focus="focus(item)" @blur="blur(item)" :disabled="(item.name!=null && (level!=0 || type!=1))?false:true">
                    ...
                    <td style="width: 14%">
                        <el-select size="small" v-model="item.type" class="param-input" @input="changeType(item)">
                    ...
                    <td style="width: 18%">
                        <el-tooltip class="item" effect="dark" :content="item.remark" placement="bottom" :disabled="!item.remark">
                            <el-input size="small" class="param-input" placeholder="请填写备注;" v-model="item.remark" @focus="focus(item)" @blur="blur(item)">
                    ...
                    <td style="width: 20%">
                        <el-tooltip class="item" effect="dark" :content="item.mock" placement="bottom" :disabled="!item.mock">
                            <el-input size="small" class="param-input"  placeholder="请填写Mock数据;" v-model="item.mock" v-if="item.type!=4" @focus="focus(item)" @blur="blur(item)">
