<template>
    <el-row class="row" style="cursor: pointer;white-space: nowrap;background-color: white;" id="tree">
        <template v-for="(item,index) in arr">
            <el-row class="row" 
                :draggable="interfaceEditRole && ((item.type==0 && session.sort!=2) || session.sort==2)" 
                style="height: 35px;line-height: 35px;white-space: nowrap" 
                :id="item.type==1?'recycle':('group'+index)" @dragover.native="dragOver($event,item)" 
                @dragleave.native="dragLeave($event)" @dragenter.native="dragEnter($event,item)" 
                @drop.native="drop($event,item,index)" 
                @dragend.native="dragEnd($event)" 
                :key="item._id" v-if="level==0 || (item.data && parent.show)" @mouseenter.native="mouseEnter($event,item)" 
                @mouseleave.native="mouseLeave($event,item)" 
                :style="{backgroundColor:item.select?'#50bfff':(item.menu?'rgb(247,246,242':'')}" 
                @dragstart.native="dragStart($event,item,index)">
                <template v-if="level>0">
                    <el-col :span="2" class="col" v-for="n in level" :key="n" :style="{'borderRight':'1px lightgray dashed'}">
                        &nbsp;
                    </el-col>
                </template>
                ...
                <div class="col" style="height: 35px;white-space: nowrap;text-align: center;position: absolute;top: 0px;right: 0px;" v-show="item.menu && interfaceEditRole && !search">
                    <el-dropdown style="height: 100%;cursor: pointer;float: right;margin-right: 3px">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <!-- <el-dropdown-item><div @click="groupTransfer(item)">迁移项目</div></el-dropdown-item> -->
                ...
                <div class="col" style="margin: 0;height: 35px;white-space: nowrap;text-align: center;position: absolute;top: 0px;right: 0px;width: 40px" v-show="item.menu">
                    <el-dropdown v-if="interfaceEditRole"  style="width: 100%;height: 100%;cursor: pointer">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item><div @click="interfaceTransfer(item,index)">迁移项目</div></el-dropdown-item>
    ...
    module.exports={
        ...
        computed:{
            ...
            arr:function () {
                if(this.level==0)
                {   
                    console.log("interfaceList.vue>arr (interfaceSearchList:interfaceList)")
                    console.log(this.$store.state.searchText)
                    console.log(this.$store.state.searchText?this.$store.state.interfaceSearchList:this.$store.state.interfaceList)
                    return this.$store.state.searchText?this.$store.state.interfaceSearchList:this.$store.state.interfaceList
                }
                else
                {   
                    console.log("interfaceList.vue>arr (this.data)")
        ...
        methods:{
            ...
            renameGroup:function (item) {
                ...
                $.input("请输入重命名的名称", (val)=> {
                    ...
                    var query={};
                    query.id=this.$store.getters.interid;
                    query.name=val.value;
                    query.group=item._id
                    $.startHud("#body");
					// console.log("interface>info>interface>component>interfaceList.vue>renameGroup-query:")
					// console.log(query)
                    this.$store.dispatch("renameGroup",query).then( data=> {
                        $.stopHud();
                        if(data.code==200)
                        {
                            $.notify("重命名成功",1);
            ...
            removeGroup:function (item) {
                ...
                $.confirm("确定删除该分组？",function () {
                    var query={
                        id:_this.$store.getters.interid,
                        group:item._id
                    }
                    $.startHud("#body");
            ...
            addGroup:function (item) {
                ...
                $.input("请输入分组名称",function (val) {
                    ...
                    var query={};
                    query.id=_this.$store.getters.interid;
                    query.name=val.value;
            ...
            groupTransfer:function (item) {
                // console.log("interfaceList.vue>groupTransfer>item")
                // console.log(item)
                var _this=this;
                $.startHud();
                var projectGroups;

                // net.get("/project/projectgroup",{}).then(function (data) {
                //     $.stopHud();
                ...
                //     var child=$.showBox(_this,require("./groupTransfer.vue"),{
                //         "projectGroups":projectGroups,
                //         "projectId":_this.$store.getters.interid,
                //         "groupId":item.group,
            ...
            interfaceTransfer:function (item,index) {
                console.log("interfaceList.vue>interfaceTransfer>index,item,")
                console.log(index)
                // console.log(item)
                var _this=this;
                $.startHud();
                ...
                net.get("/project/projectgroup",{}).then(function (data) {
                    ...
                    var child=$.showBox(_this,require("./interfaceTransfer.vue"),{
                        "projectGroups":projectGroups,
                        "projectId":_this.$store.getters.interid,
                        "groupId":item.group,
                        "interfaceId":item._id
                    ...
                    child.$on("save",function (data) {
                        // console.log('child.save>data')
                        // console.log(data)
                         var interfaceList=_this.$store.state.searchText?_this.$store.state.interfaceSearchList:_this.$store.state.interfaceList;
                        if (data.group!=item.group) {
                           
                            if (data.project==_this.$store.getters.interid) {
                                for (let obj of interfaceList) {
                                    if (obj._id.toString()==item.group) {
                                        obj.data.splice(index,1)
                                    }
                                    if (obj._id.toString()==data.group) {
                                        obj.data.push(item)
                                    }
                                }
                            } else {
                        ...
                        this.$store.dispatch("refreshData",interfaceList);
