        ...
        <el-row class="row" style="height: 100%;border-top: 1px lightgray solid;">
            <el-col class="col" :span="5" style="overflow-y: auto;border-right: 1px lightgray solid;" id="docRefList">
                <template v-for="item in arrList">
                    <el-row  :key="item.name" class="row docRefListHover" style="height: 35px;line-height: 35px;text-align: center;cursor: pointer;border-bottom: 1px solid lightgray" :style="{backgroundColor:item.select?'#f3f3f3':'white'}" @click.native="info(item)">
                        {{item.name}}
                    </el-row>
                </template>
            </el-col>
...
<script>
    ...
    var showInterface=require("../../../../document/component/interface.vue");
