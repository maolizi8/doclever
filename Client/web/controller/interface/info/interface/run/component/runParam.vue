<template>
    <el-row class="row">
        <el-row class="row" style="background-color: white;" id="paramContent">
            ...
            <expand v-if="interfaceObj.method=='POST' || interfaceObj.method=='PUT' || interfaceObj.method=='PATCH'" ref="body" :expand="bodyAutoExpand">
        ...
        <el-row class="row" style="margin-bottom: 20px;">
            <el-row class="row" style="text-align: center;height: 36px;line-height: 36px" v-if="example.id">
                    ...
                    <el-tabs type="border-card" id="result">
                        <el-tab-pane label="Preview">
                            <el-row class="row" v-if="type=='object'">
                                <template v-for="(item,index) in draw">
                                    <el-row class="row" style="font-size: 14px;min-height: 25px;line-height: 25px;word-break: break-all" v-html="item" :key="index">
                                    </el-row>
                                </template>
                            </el-row>
                            <img :src="imgUrl" v-else-if="type=='img'">
                            <!-- <div v-html="draw" v-else-if="type=='html'"></div> -->
                             <div v-else-if="selParam.type=='html'" style="word-break: break-all">
                                {{rawData}}
                            </div>
                        ...
                        <el-tab-pane label="Advance">
                            <el-row class="row" v-if="type=='object'">
                                <!-- <template v-for="(item,index) in drawMix" v-if="type=='object'" > -->
                                <template v-for="(item,index) in drawMix">
                                    <el-popover placement="bottom" title="错误信息" width="300" trigger="hover" v-if="existError(item)" :key="index">
                                        {{getError(item)}}
                                    ...
                                    <el-row class="row" style="font-size: 14px;min-height: 25px;line-height: 25px;word-break: break-all" v-html="item" v-else :key="index">
                                    </el-row>
                                </template>
                            </el-row>
                            <img :src="imgUrl" v-else-if="type=='img'">
            ...
            <template v-else-if="resultType==0 && selParam.run">
                <el-row class="row" style="padding: 10px 10px 10px 20px;margin-bottom: 10px" key="example">
                    <span>
                        <span style="font-weight:bold;">Result:</span>&nbsp;&nbsp;<span :style="{color:selParam.status.match(/^2/)?'green':'red'}">{{selParam.status=='0'?'ERROR':selParam.status}}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color: #50a3ff">{{selParam.second?("耗时"+selParam.second+"秒"):""}}</span>
                    </span>
                </el-row>
                    ...
                    <el-tabs type="border-card" id="result">
                        <el-tab-pane label="Preview">
                            <el-row class="row" v-if="selParam.type=='object'">
                                <template v-for="(item,index) in selParam.draw">
                                    <el-row class="row" style="font-size: 14px;min-height: 25px;line-height: 25px;word-break: break-all" v-html="item" :key="index">
                                    </el-row>
                                </template>
                            </el-row>
                        ...
                        <el-tab-pane label="Response Header">
                            <table class="table-hover" style="width: 100%">
                                ...
                                <template v-for="(value,key) in selParam.resHeader">
                                    <tr style="vertical-align: middle;height: 30px;table-layout: fixed" :key="key">
                                        <td style="width: 30%">
                                            {{key}}
                                        </td>
                                        <td style="width: 70%;word-wrap:break-word">
