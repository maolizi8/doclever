                ...
                <tr style="text-align: center;vertical-align: middle;cursor: move;height: 36px;line-height: 36px" :draggable="item.drag?item.drag:'false'" @dragover="dragOver($event,item)" @dragleave="dragLeave($event,item)" @dragenter="dragEnter($event,item)" @drop="drop($event,item,index,arr)" @dragstart="dragStart($event,item,index,arr)" @dragend="dragEnd($event)" :key="index">
                    <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
                        ...
                        <el-col class="col" :span="22">
                            <el-input size="small" class="param-input"  placeholder="请填写名称" v-model.trim="item.name" v-if="item.name!=null && (level!=0 || type!=1)" @focus="focus(item)" @blur="blur(item)"></el-input>
                            <el-input size="small" class="param-input"  placeholder="该字段没有名称" disabled v-else></el-input>
                        </el-col>
                    </td>
                    <td style="width: 14%">
                    ...
                    <td style="width: 18%;line-height: normal;    word-break: break-word;">
