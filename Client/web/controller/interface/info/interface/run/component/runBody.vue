<template>
    <el-row class="row">
        <el-row class="row" style="padding:0 0 0 20px;height: 32px;line-height: 32px;white-space: nowrap">
        ...
        <table width="100%" id="bodyTable" v-show="info.type==0">
            ...
            <template v-for="(item,index) in arr">
                <tr :style="{height: '36px',lineHeight: '36px',textAlign: 'center',verticalAlign: 'middle',backgroundColor:item.enable?'white':'lightgray'}" :key="index">
                    ...
                    <td style="width: 25%;text-align: center;vertical-align: middle;overflow: auto;">
                        ...
                        <el-input size="small" class="param-input"  placeholder="请填写值" v-model="item.selValue" v-else-if="item.type==0" custom></el-input>
                    ...
                    <td style="width: 5%">
                        <el-button type="text" size="mini" style="font-size: 15px;" @click="toggleEnable(item,index)"><span :class="item.enable?'fa fa-eye':'fa fa-eye-slash'" :title="item.enable?'运行时包含此字段':'运行时不包含此字段'"></span></el-button>
                    </td>
                    <td style="width: 5%">
