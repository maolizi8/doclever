<template>
    <el-row class="row" style="padding: 10px 10px 60px 10px;height: calc(100vh - 115px);overflow-y: auto;font-size: 14px" id="globalInfo">
        <expand class="row expend_head" style="" id="projectBasicInfo">
            ...
            <el-row class="row" style="padding: 10px 20px 10px 20px">
                <el-form ref="form" label-width="100px" label-position="top">
                    ...
                    <el-row class="row" style="margin-top: 10px">
                        ...
                        <!-- <el-button size="mini" type="danger" @click.prevent="removeProject" :loading="deletePending" v-if="!guestRole && ownRole">
                            删除项目
                        </el-button> -->
                        <el-button size="mini" type="danger" @click.prevent="outofProject" :loading="outofPending" v-if="!guestRole && !ownRole">
                            退出项目
                        </el-button>
        ...
        <expand class="row expend_head" style="" id="statusEdit">
            <el-row class="row" style="height: 40px;line-height: 40px;color: #17b9e6" slot="title">
                修改项目成员
            </el-row>
            
            <el-row class="row" style="padding: 5px 20px 10px 20px">
                <el-row class="row" style="height: 40px;text-align: center;line-height: 40px;" v-if="manageRole">
                    <el-col class="col" :span="2" style="white-space: nowrap">
                        添加用户：
                    </el-col>
                    <el-col class="col" :span="4">
                        <el-input size="small" placeholder="输入待添加的用户名" style="width: 90%" v-model="name"></el-input>
                    </el-col>
                    ...
                    <el-col class="col" :span="3" style="line-height: 40px">
                        <el-button  size="mini" @click="editRoleOption" type="text" v-if="role==1">权限设置</el-button>
                    </el-col>
                    <el-col class="col" :span="2" style="line-height: 40px;">
        ...
        <expand class="row expend_head" style="">
            <el-row class="row" style="height: 40px;line-height: 40px;color: #17b9e6" slot="title">
                导出
            </el-row>
            <el-row class="row" style="background-color: lightgray;height: 1px"></el-row>
        ...
        <expand class="row expend_head" style="" v-if="manageRole">
        ...
        <expand class="row expend_head" style=""  v-if="project.source && manageRole">
            <el-row class="row" style="height: 40px;line-height: 40px;color: #17b9e6" slot="title">
                更新工程
            </el-row>
            <el-row class="row" style="background-color: lightgray;height: 1px"></el-row>
 ...
 */
    ...
    var store=require("../../store")._modulesNamespaceMap["info/setting/"].context;
    ...
    module.exports={
        ...
        computed:{
            publicUrl:function () {
                ...
                return url.substring(0,index+1)+"public/public.html#"+this.interid;
            },
            guestRole:function () {
            ...
            mockUrl:function () {
                if(session.get("versionId"))
                {
                    ...
                    return config.projectBaseUrl+"/mock/"+this.interid+session.get("versionId")
                }
                else
                {
                    return config.projectBaseUrl+"/mock/"+this.interid
                }
            }
        },
        components:{
        ...
        methods:{
            ...
            outofProject:function () {
                var _this=this;
                $.confirm("确定退出该工程？",function () {
                    _this.outofPending=true;
                    net.delete("/project/quit",{
                        
                        id:_this.interid,
                    }).then(function (data) {
                        _this.outofPending=false;
                        if(data.code==200)
                        {
                            $.notify("退出成功",1);
                            window.open("team.html",'_self')
            ...
            exportJSON:function () {
                ...
                {
                    window.open(location.protocol+"//"+location.host+"/project/"+(this.exportType==0?"exportjson":(this.exportType==1?"exporthtml":"exportdocx"))+"?id="+this.interid);
                }
                else
                {
                    var link=document.createElement("a");
                    link.href="/project/"+(this.exportType==0?"exportjson":(this.exportType==1?"exporthtml":"exportdocx"))+"?id="+this.interid;
